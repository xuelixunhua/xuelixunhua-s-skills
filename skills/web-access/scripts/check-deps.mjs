...
const RESET_PROXY_HINT = os.platform() === 'win32'
  ? "PowerShell: Get-CimInstance Win32_Process -Filter \"Name = 'node.exe'\" | Where-Object { $_.CommandLine -like '*cdp-proxy.mjs*' } | ForEach-Object { Stop-Process -Id $_.ProcessId }"
...
async function ensureProxy(expectedBrowserId, browserOverride) {
  ...
  if (health?.status === 'ok' && health.connected) {
    ...
    if (expectedBrowserId && runningId && runningId !== 'unknown' && runningId !== expectedBrowserId) {
      ...
      console.log(`  请先运行 ${RESET_PROXY_HINT} 重置后再试`);
      return false;
    }
